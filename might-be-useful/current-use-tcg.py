...
class TCGdexCardExtractorWithPrices:
    ...
    def __init__(self, input_csv: str, language: str = "en", output_dir: str = "tcg_output", 
                 enable_timeout: bool = False, timeout_seconds: int = 30):
        """
        ...
        Args:
            ...
            enable_timeout: Whether to enable timeout for image downloads (default: False)
    ...
    def extract_prices(self, card) -> List[str]:
        ...
        Returns:
            List of price strings (e.g., ["$7.50", "$14.99", "$3.25"])
        """
        prices = []
        ...
        try:
            ...
            if prices:
                print(f"  ✓ Found {len(prices)} price(s): {prices}")
            else:
    ...
    def format_price_list(self, prices: List[str]) -> str:
        """
        Format price list as a comma-separated string.
        
        Args:
            prices: List of price strings
            
        Returns:
            Formatted price string (e.g., "$7, $14, $3")
        """
        if not prices:
            return ""
        return ", ".join(prices)
    
    def extract_card_data(self, card, original_name: str, image_filename: Optional[str]) -> Dict:
        """
        Extract relevant data from a card object.
        
        Args:
            card: Card object from TCGdex
            original_name: Original card name from input CSV
        ...
        try:
            ...
            # Format prices
            prices_str = self.format_price_list(prices)
    ...
    async def process_single_card_name(self, card_name: str, download_images: bool = True) -> List[Dict]:
        """
        Process a single card name and return all variants.
        ...
        Returns:
            List of dictionaries containing card data for all variants
        """
        results = []
        ...
        try:
            ...
            if not matching_cards:
                print(f"  ⚠ No matches found for '{card_name}'")
            ...
            for card_idx, card in enumerate(matching_cards, 1):
                ...
                # Extract card data
                card_data = self.extract_card_data(detailed_card, card_name, image_filename)
    ...
    async def process_card_names(self, card_names: List[str], download_images: bool = True) -> List[Dict]:
        ...
        for idx, card_name in enumerate(card_names, 1):
            ...
            # Process this card name and get all variants
            card_results = await self.process_single_card_name(card_name, download_images)
            processed_data.extend(card_results)
    ...
    async def extract_cards_from_csv(self, download_images: bool = True) -> None:
        ...
        try:
            ...
            print("=" * 60)
            print(f"Timeout Feature: {'ENABLED' if self.enable_timeout else 'DISABLED'}")
            if self.enable_timeout:
                print(f"Timeout Duration: {self.timeout_seconds} seconds")
            print("=" * 60)
...
async def main():
    ...
    try:
        ...
        extractor = TCGdexCardExtractorWithPrices(
            ...
            language="en",
            output_dir="tcg_data",
            enable_timeout=False,  # Set to True to enable timeout
