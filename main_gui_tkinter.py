...
from yuyutei_gui import YuyuTeiGUI
...
class TCGScraperGUI:
    ...
    def create_control_buttons(self, parent):
        ...
        self.upload_btn = ModernButton(btn_container, "UPLOAD", 
                                       ...
                                       icon="📤")
        self.upload_btn.pack(side=tk.LEFT, fill=tk.X, expand=True, padx=(0, 10))
        self.upload_btn.config(width=150)
        
        # Back button (gray)
        self.back_btn = ModernButton(btn_container, "BACK", 
                                     self.go_back_to_selection, '#6b7280', '#4b5563',
                                     icon="◀")
        self.back_btn.pack(side=tk.LEFT, fill=tk.X, expand=True)
        self.back_btn.config(width=150)
    ...
    def _on_resize(self, event):
        ...
        if hasattr(self, 'start_btn'):
            ...
            self.back_btn.draw_button(self.back_btn.bg_color)
...
"""
    ...
    def open_upload_window(self):
        ...
        # Open upload window (TCG source)
        UploadWindow(self.root, source='tcg')
    ...
    def go_back_to_selection(self):
        ...
        if messagebox.askyesno("Confirm", "Are you sure you want to go back to the selection window?"):
            ...
            SelectionWindow(root, launch_tcg_gui, launch_yuyutei_gui)
            root.mainloop()


def launch_tcg_gui():
    """Launch TCG CSV scraper GUI"""
    root = tk.Tk()
...
def launch_yuyutei_gui():
    """Launch Yuyu-Tei scraper GUI"""
    root = tk.Tk()
...
def main():
    """Main entry point - show selection window"""
    root = tk.Tk()
    
    # Set application icon for selection window
    try:
        icon_path = os.path.join(os.path.dirname(__file__), 'icon.ico')
        if os.path.exists(icon_path):
            root.iconbitmap(icon_path)
    ...
    SelectionWindow(root, on_tcg_selected=launch_tcg_gui, on_yuyutei_selected=launch_yuyutei_gui)
    root.mainloop()
